...
| 2 | **Mule items (scriptures/grimoires)** | **Yes** | High | Set `bots_mule_pickup = true` on pickup templates via `require()` mutation (same pattern as `ability_meta_data` injection). `BotGroup` scanning, `can_loot` condition, interact action, and destination priority 5 all activate automatically. | None — drop-in fix |
