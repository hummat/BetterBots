...
**Ballistic aim status:** BetterBots grenade fallback now mirrors vanilla `Trajectory.angle_to_hit_moving_target(...)` solving for supported gravity-affected manual-physics projectile families. Covered: standard grenades, handleless grenades, Ogryn grenade throws, and zealot throwing knives. Excluded: `broker_missile` (near-flat), psyker knives (true-flight), whistle, smite, chain lightning, and mines. The old “all knives unaffected” note was wrong for zealot knives.
