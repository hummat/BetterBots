...
**Target node** (`_aim_position` lines 276-286): Aim point resolves through `target_breed.override_bot_target_node` → `scratchpad.aim_at_node` (from `attack_meta_data.aim_at_node`) → hardcoded `"j_spine"`. Charged shots use the parallel `aim_at_node_charged` chain. The `aim_at_node` field can be a table → randomized per target acquisition. **Vanilla has only one head-aim exception**: `high_bot_lasgun_killshot.lua:738` sets `aim_at_node = { "j_head", "j_spine" }`. Every other vanilla bot weapon — and every player weapon — falls back to `j_spine` (center of mass), so vanilla bots never weakspot-target. BetterBots `#91` now reuses that same head/spine table for an allowlist in `ranged_meta_data.lua` (lasguns, autoguns, bolters/bolt pistols, stub revolvers, plasma guns, and both twin-linked `ogryn_heavystubber_p1_*` and single `ogryn_heavystubber_p2_*` heavy stubbers) when the template leaves `aim_at_node` unset. BetterBots `#92` then post-processes `_set_new_aim_target`: Scab Mauler is pinned to `j_spine`, Bulwark is pinned to `j_head` only when its shield is open or the bot is outside the 70° blocking cone, and Crusher is **provisionally** pinned to `j_head` only from the rear arc as a proxy for the issue's back-of-head claim. That Crusher route is assumption, not rig-verified fact; the decompiled source still does not expose a confirmed back-head node name. Breed `override_bot_target_node` still only redirects aim toward bodies on oddly-shaped enemies (dogs, monstrosities), never toward heads.
