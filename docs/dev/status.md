...
- **v0.11.0 is not ready to tag yet** — `make check` is green (`943 successes / 0 failures / 0 errors`), but the release still has open validation gates: `#32`, `#82`, `#89`, `#90`, plus daemonhost re-validation on `#17`.
...
- **v0.11.0 "Combat Execution" (final polish batch)**: code complete on `dev/v0.11.0`, but still validation-gated. `#44`, `#87`, `#91`, and `#93` closed on 2026-04-13; `#32`, `#82`, `#89`, and `#90` remain open because the current logs still do not prove tome/scripture pickup, fresh `/bb_perf` evidence, standalone `small_grenade` pickup consumption, or target-type hysteresis flips/holds in longer combat.
