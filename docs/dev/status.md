...
- **v0.11.0 "Combat Execution" (final polish batch)**: code complete on `dev/v0.11.0`, but still validation-gated. `#44`, `#87`, `#89`, `#91`, and `#93` are now closed; `#32`, `#82`, and `#90` remain open. `#32` now has repo-level code for both metadata repair and a post-vanilla mule-assignment override so nearby humans no longer suppress default book pickup, but the issue still needs a real in-game tome/scripture pickup confirmation before closure. `#90` now also has a BT-side opposite-type reswitch debounce on top of the earlier perception-layer hysteresis, but it still needs a fresh combat log showing reduced switch churn before closure.
