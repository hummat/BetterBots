...
- **v1.0.0 "Bot Identity" (terminal release)**: staggered 6-sprint plan (see `docs/dev/roadmap.md` v1.0.0 section). Sprint 1: F1 talent context landed, `#92` is now code-complete for the current scope (Scab Mauler spine override, Bulwark exposed-head routing, plus a **provisional** Crusher rear-arc head proxy documented as assumption rather than verified rig fact), and `#86` is **done 2026-04-17** (`BtBotInteractAction.enter` hook confirmed unusable, Tier 3 implementation moved to Post-1.0). The remaining Sprint 1 holdout is `#13` (navmesh charge validation). Sprint 2: #38 full shipped-roster keystone layer (Zealot Martyrdom, Psyker Warp Siphon/glass cannon, Psyker Venting Shriek, Veteran VoC). Sprint 3: narrow #41 (close-range weapon-family classifier) + narrow #33 (activate_special melee). Sprint 4: F2 pocketable pickup primitive + #24 (medicae/stim/med-kit) + #88 (crate carry+deploy). Sprint 5: #56 (com wheel) + #96 (smart-tag bridge) + #97 (resource arbitration) + #101 (weapon-slot wield timeout). Sprint 6: #98 (sparse metadata hardening) + #99 (perf benchmark protocol) + #85 (ability identity refactor) + release soak.
