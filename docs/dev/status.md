...
- `#107` is code-complete, not live-validated. The branch now adds a general movement-safety layer: fused barrels emit bot AoE threats from `c_explosion` with a configurable buffer, pending dodges are cancelled when the projected endpoint fails nav/drop checks, ordinary movement fails open after stairs/downhill false positives, and passive daemonhosts have a tunable keepout steering radius. The next live pass should use barrels, floor hazards, ledges, stairs, and `/bb_scenario daemonhost_passive_near`, then check for `hazard_prop buffered threat`, `aoe_threat consumed`, `movement safety blocked`, and `movement safety steered away from daemonhost`.
