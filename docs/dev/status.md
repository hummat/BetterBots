...
- **v1.0.0 "Bot Identity" (terminal release)**: staggered 6-sprint plan (see `docs/dev/roadmap.md` v1.0.0 section). Sprint 1 is now code-complete: F1 talent context landed, `#13` landed as a shared charge/dash nav validator (`navigation_extension:destination()` + `NavQueries.ray_can_go(...)`, wired in both BT enter and fallback with a 0.5 s same-destination negative cache), `#92` is code-complete for the current scope (Scab Mauler spine override, Bulwark exposed-head routing, plus a **provisional** Crusher rear-arc head proxy documented as assumption rather than verified rig fact), and `#86` is **done 2026-04-17** (`BtBotInteractAction.enter` hook confirmed unusable, Tier 3 implementation moved to Post-1.0). Sprint 2: #38 full shipped-roster keystone layer (Zealot Martyrdom, Psyker Warp Siphon/glass cannon, Psyker Venting Shriek, Veteran VoC). Sprint 3: narrow #41 (close-range weapon-family classifier) + narrow #33 (activate_special melee). Sprint 4: F2 pocketable pickup primitive + #24 (medicae/stim/med-kit) + #88 (crate carry+deploy). Sprint 5: #56 (com wheel) + #96 (smart-tag bridge) + #97 (resource arbitration) + #101 (weapon-slot wield timeout). Sprint 6: #98 (sparse metadata hardening) + #99 (perf benchmark protocol) + #85 (ability identity refactor) + release soak.
