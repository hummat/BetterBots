...
| — | Bot build overhaul | **Closed** | All 4 class profiles were redesigned and then retuned again on 2026-04-20 to a validation-first shipped lineup. Current defaults: Veteran = VoC + Focus Target + precision lasgun + chainsword; Zealot = Fury + Martyrdom + chainaxe + stub revolver; Psyker = Scrier's + Brain Rupture + electro staff + force sword; Ogryn = Point-Blank Barrage + armor-pen + rippergun + Bully Club. The shipped profile source now authors T4 weapon perks/blessings for both weapon slots on all 4 classes, and it declares a conservative universal curio package for downstream consumers: 3× Blessed Bullet with T4 toughness, combat ability regen, gunner DR, and stamina regeneration. BetterBots also now has attachment-slot runtime plumbing for authored curios, but the concrete curio `master_item_id`s are still missing, so full runtime curio spawning is still pending a live `curio_dump`. In-game loading confirmed; full mission validation of the new lineup is still pending. |
