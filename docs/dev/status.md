...
2. **#17 daemonhost avoidance / #107 hazard movement safety**: v0.6.0 suppression first had a heuristic-path gap (grenade_fallback bypassed BT condition wrappers), then a state-model gap (`aggro_state` only was too late once the daemonhost started waking), then a scope gap (offensive abilities were still allowed near a sleeping daemonhost unless the DH had already become `target_enemy`), then a direct queue gap (weapon/grenade `weapon_action` inputs still fired once vanilla selected a daemonhost target), then a known-enemy gap (scenario-spawned passive daemonhosts were not active AI targets, so the proximity scan missed them). The current branch now uses daemonhost `stage` when available, preserves the long-range target-based carve-out, restores a proximity gate for offensive abilities plus close-range melee/ranged checks, blocks central `weapon_action` inputs against non-aggroed daemonhost `target_enemy`, scans the bot side's known enemy relation list and the minion spawn manager's live minion list for passive daemonhost proximity, blocks abilities/blitzes whose current target stands inside the daemonhost keepout radius, blocks ranged fire at non-daemonhost targets inside that keepout zone, and steers bots away from passive daemonhosts inside `daemonhost_keepout_distance`. The same movement-safety pass also emits buffered barrel threats at the explosion node and blocks pending dodges off unsafe nav endpoints; ordinary movement no longer cancels on the generic ledge endpoint check because live stairs/downhill traversal produced thousands of false `ledge_drop` blocks. Watch for `*_block_dormant_daemonhost`, `ability suppressed (daemonhost_nearby)`, `daemonhost_nearby_target`, `melee/ranged suppressed (... daemonhost ...)`, `ranged suppressed (target near dormant daemonhost)`, `blocked foreign weapon action ... daemonhost_avoidance ... dormant=true`, `movement safety steered away from daemonhost`, `hazard_prop buffered threat`, `aoe_threat consumed`, `movement safety blocked`, `ability allowed against daemonhost`, `dormant_daemonhost` skip lines, and daemonhost `stage`/`aggro_state` around the first target/action.
...
- `#107` is code-complete, not live-validated. The branch now adds a general movement-safety layer: fused barrels emit bot AoE threats from `c_explosion` with a configurable buffer, pending dodges are cancelled when the projected endpoint fails nav/drop checks, ordinary movement fails open after stairs/downhill false positives, and passive daemonhosts have a tunable keepout steering radius backed by the minion spawn manager's live minion list. The next live pass should use barrels, floor hazards, ledges, stairs, and `/bb_scenario daemonhost_passive_near`, then check for `hazard_prop buffered threat`, `aoe_threat consumed`, `movement safety blocked`, `movement safety steered away from daemonhost`, `ranged suppressed (target near dormant daemonhost)`, and `daemonhost_nearby_target`.
