...
- **Daemonhost avoidance** (#17): suppress bot combat near non-aggroed daemonhosts. The current branch has two gates: daemonhost `stage` is authoritative when available for target-based suppression, and offensive abilities plus close-range melee/ranged checks also bail out on a tight daemonhost proximity radius. Later logs from 2026-04-23 and 2026-04-24 show daemonhost spawns and bot engagements, but no positive dormant-suppression evidence; the 2026-04-24 engagement is inconclusive because BetterBots classified the target as non-dormant before bots attacked it.
...
| — | Bot build overhaul | **Closed** | All 4 class profiles were redesigned and then retuned again on 2026-04-20 to a validation-first shipped lineup. Current defaults: Veteran = VoC + Focus Target + precision lasgun + chainsword; Zealot = Fury + Martyrdom + chainaxe + stub revolver; Psyker = Scrier's + Brain Rupture + electro staff + force sword; Ogryn = Point-Blank Barrage + Kickback + latrine shovel. The shipped profile source now authors T4 weapon perks/blessings for both weapon slots on all 4 classes, and it authors a conservative universal curio package for both downstream consumers and runtime equip: 3× Blessed Bullet (Reliquary) on the live dumped gadget base `content/items/gadgets/defensive_gadget_11`, with T4 toughness, combat ability regen, gunner DR, and stamina regeneration. In-game loading confirmed; full mission validation of the new lineup is still pending. |
...
- `#100` first harness slice is code-complete and refined after the 2026-04-29 smoke run: `/bb_scenario` can spawn `poxburster_push`, `crusher_pack`, and `mauler_weakspot`, optional `[distance] [count]` arguments can repeat/spread spawns for stress tests, JSONL scenario rows are emitted, and `/bb_scenario_clear` clears tracked spawned units. The 2026-04-29 19:11 log proves repeated `mauler_weakspot` spawns honored distance `22` and count `10`.
- `#92` Mauler scenario evidence improved on 2026-04-29: bolter/heavy-stubber bots kept ranged target type at long Mauler distance, switched back to melee around the 12m anti-armor cutoff, the heavy-stubber bash loop did not recur, and Zealot knives only produced hard-armor hold logs. Generic weakspot validation is still open because the run showed only `weakspot override applied (breed=renegade_executor, node=j_spine)`, not `weakspot aim selected`.
