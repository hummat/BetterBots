...
| #17 | Daemonhost avoidance | **Mixed-target keepout gap fixed; live re-test pending** | First DH spawn in v0.10.0 validation exposed the heuristic-path gap: `grenade_fallback` -> `_grenade_priority_target` bypassed the `condition_patch` BT wrappers, `psyker_smite` fired on dormant DH at 19m. Later runs exposed state-model, direct queue, known-enemy, and mixed-target keepout gaps. Current branch now makes daemonhost `stage` authoritative when available, blocks direct `weapon_action` inputs against non-aggroed daemonhost targets, scans `MinionSpawnManager:spawned_minions()` even when side lookup is stale, blocks ranged/grenade/blitz decisions whose target is inside DH keepout, and blocks charge/dash endpoints inside keepout. Next live run should look for `daemonhost scan source`, `daemonhost scan candidate`, `target near daemonhost scan`, `ranged suppressed (target near dormant daemonhost)`, `daemonhost_nearby_target`, `fallback blocked <charge_template> (charge_nav=daemonhost_target_near)`, and `target_is_near_dormant_daemonhost = true`. |
...
2. **#17 daemonhost avoidance / #107 hazard movement safety**: v0.6.0 suppression first had a heuristic-path gap (grenade_fallback bypassed BT condition wrappers), then a state-model gap (`aggro_state` only was too late once the daemonhost started waking), then a scope gap (offensive abilities were still allowed near a sleeping daemonhost unless the DH had already become `target_enemy`), then a direct queue gap (weapon/grenade `weapon_action` inputs still fired once vanilla selected a daemonhost target), then a known-enemy gap (scenario-spawned passive daemonhosts were not active AI targets, so the proximity scan missed them), then a mixed-target gap (Krak/dash toward a Mauler inside the awake-but-not-aggroed DH zone). The current branch now uses daemonhost `stage` when available, preserves the long-range target-based carve-out, restores a proximity gate for offensive abilities plus close-range ranged checks, blocks central `weapon_action` inputs against non-aggroed daemonhost `target_enemy`, scans the bot side's known enemy relation list and the minion spawn manager's live minion list for passive/awake-but-not-aggroed daemonhost proximity even when side lookup is missing, logs daemonhost scan source/candidate classification, logs target-to-daemonhost range buckets when a bot has a current target and any non-aggroed daemonhost is known, blocks abilities/blitzes whose current target stands inside the daemonhost keepout radius, blocks ranged fire and charge/dash endpoints at non-daemonhost targets inside that keepout zone, keeps mixed-target melee available, and softly biases bot movement away inside the tighter movement radius. The same movement-safety pass also emits buffered barrel threats at the explosion node and blocks pending dodges off unsafe nav endpoints; ordinary movement no longer cancels on the generic ledge endpoint check because live stairs/downhill traversal produced thousands of false `ledge_drop` blocks. Watch for `daemonhost scan source`, `daemonhost scan candidate`, `target near daemonhost scan`, `*_block_dormant_daemonhost`, `ability suppressed (daemonhost_nearby)`, `daemonhost_nearby_target`, `melee/ranged suppressed (... daemonhost target ...)`, `ranged suppressed (target near dormant daemonhost)`, `fallback blocked <charge_template> (charge_nav=daemonhost_target_near)`, `blocked foreign weapon action ... daemonhost_avoidance ... dormant=true`, `movement safety steered away from daemonhost`, `hazard_prop buffered threat`, `aoe_threat consumed`, `movement safety blocked`, `ability allowed against daemonhost`, `target_is_near_dormant_daemonhost`, `dormant_daemonhost` skip lines, and daemonhost `stage`/`aggro_state` around the first target/action.
...
- `#107` is code-complete, not live-validated. The branch now adds a general movement-safety layer: fused barrels emit bot AoE threats from `c_explosion` with a configurable buffer, pending dodges are cancelled when the projected endpoint fails nav/drop checks, ordinary movement fails open after stairs/downhill false positives, and passive/awake-but-not-aggroed daemonhosts have soft close-range movement steering while the configurable keepout radius remains the action-suppression zone. The next live pass should use barrels, floor hazards, ledges, stairs, and `/bb_scenario daemonhost_passive_near`, then check for `hazard_prop buffered threat`, `aoe_threat consumed`, `movement safety blocked`, `movement safety steered away from daemonhost`, `target near daemonhost scan`, `ranged suppressed (target near dormant daemonhost)`, `fallback blocked <charge_template> (charge_nav=daemonhost_target_near)`, and `daemonhost_nearby_target`.
