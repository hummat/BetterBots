...
- **v0.11.0 "Combat Execution" (final polish batch)**: released on `2026-04-15`, and the issue cleanup is complete. `#17`, `#32`, `#44`, `#82`, `#87`, `#89`, `#90`, `#91`, and `#93` are closed. `#17` is closed on the 2026-05-02 targeted daemonhost run; `#32` is closed on authoritative runtime evidence from `console-2026-04-15-18.01.55...` (`mule pickup success: tome (bot=4)`). `#82` is closed on direct 2026-04-16 runtime proof that the final same-frame `resolve_decision(...)` reuse landed, with the same log also showing a mission-end `104.9 us/bot/frame` sample; any deeper perf work is now tracked separately in `#99`. `#90` is closed on direct 2026-04-16 runtime proof from `console-2026-04-16-17.51.56...`: the fixed `BotPerceptionExtension._update_target_enemy` hook produced `32` `type flip ...` lines, `46` `type hold ...` lines, `4` `suppressed opposite-type switch ...` lines, and the mission-end row `target_type_hysteresis.post_process 143.000 ms total (11473 calls, 12.5 us/call)`. |
...
- `#17` is closed from `console-2026-05-02-14.33.58...`: passive and awake-but-not-aggroed daemonhost states were detected, close movement was softly steered away, ranged fire at nearby non-DH targets was suppressed, and grenade/blitz/ability decisions held with `daemonhost_nearby_target`.
...
- `#100` is closed. `/bb_scenario mauler_weakspot 22 10` emitted three `scenario_start` runs, 30 `scenario_spawn` rows for `renegade_executor`, and three `scenario_result status=spawned` rows. The scenario library now also includes `mixed_horde_pressure`, `daemonhost_passive_near`, and `daemonhost_aggroed_control`.
...
- `#107` is closed from `console-2026-05-02-14.33.58...`: hazard props triggered, AoE threats were accepted and consumed by bot movement, ledge endpoint blocking fired, and daemonhost movement steering was validated in the same movement-safety pass.
- `#96` is closed from `console-2026-05-02-14.33.58...`: explicit smart-tag item interactions produced repeated `smart-tag pickup routed ... to bot ... (family=slot_order)` lines. The follow-up `a8453fb` commit guards the duplicate `BotOrder.pickup` hook warning found in that run.
