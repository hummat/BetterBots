...
- **v1.0.0 "Bot Identity" (terminal release)**: staggered 6-sprint plan (see `docs/dev/roadmap.md` v1.0.0 section). Sprint 1: F1 talent context + #13 (navmesh charges) + #92 (per-breed weakspot) + #86 timing investigation (**done 2026-04-17**: `BtBotInteractAction.enter` hook confirmed unusable, Tier 3 implementation moved to Post-1.0). Sprint 2: #38 full shipped-roster keystone layer (Zealot Martyrdom, Psyker Warp Siphon/glass cannon, Psyker Venting Shriek, Veteran VoC). Sprint 3: narrow #41 (close-range weapon-family classifier) + narrow #33 (activate_special melee). Sprint 4: F2 pocketable pickup primitive + #24 (medicae/stim/med-kit) + #88 (crate carry+deploy). Sprint 5: #56 (com wheel) + #96 (smart-tag bridge) + #97 (resource arbitration) + #101 (weapon-slot wield timeout). Sprint 6: #98 (sparse metadata hardening) + #99 (perf benchmark protocol) + #85 (ability identity refactor) + release soak.
- **Post-1.0 "Intelligence Architecture" (may never ship)**: #22 (utility scoring), #28 (profile management), #80 (grenade tactical evaluator), #84 (user-authored profiles), #86 (Tier 3 revive cover — moved 2026-04-17 after enter-hook scope-exit; approach-phase architecture via `on_refresh_destination` + gated `can_revive` documented on the issue). Broad-scope cuts from v1.0.0 issues (broad #24, broad #33, broad #41, broad #92, keystone extensions beyond shipped roster) formally scope-exited under the parent issues.
