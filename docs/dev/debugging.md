...
| `bot compensation profile base-game` / `bot compensation profile override` / `bot compensation profile buff-suppressed` | Bot survivability compensation selector resolved to the base-game identifier, a BetterBots override, or the temporary spawn-buff suppression value. Logged once per identifier when debug logging is enabled. |
...
| `animation guard fell back to plain anim_event for` | Bot animation attempted `anim_event_with_variable_float` with a missing/invalid animation variable, so BetterBots emitted the plain event instead. This is expected fallback behavior and is debug-only, not a DMF warning. |
| `suppressed failed bot anim_event` | Stingray rejected a bot's `Unit.animation_event` call. BetterBots suppressed the API error to keep the session alive. This is a defect signal: check what slot the bot had wielded. |
| `released grenade weapon lock for interaction during <grenade>` | An interaction began during an active grenade sequence, and the grenade lock yielded before the interaction wielded `slot_unarmed` or `slot_device`. |
...
| `grenade pickup permitted: all eligible humans above reserve` / `grenade pickup permitted: human smart-tag order` | BetterBots reserved a world grenade pickup for the bot either through normal reserve arbitration or an explicit human smart-tag; reserve-arbitrated lines include `lowest_human_grenades=<current>/<max>, threshold=<pct>` when debug logging is enabled; one-shot per bot+pickup reservation episode |
| `grenade pickup bound into ammo slot` / `grenade pickup bound into ammo slot from human smart-tag` | BetterBots attached the reserved world grenade pickup to the ammo-pickup fields so vanilla interaction code can collect it; the smart-tag variant proves an explicit tag survived the later reserve-policy pass |
| `grenade pickup deferred to human reserve` | Bot yielded a nearby world grenade pickup because at least one eligible human grenade user was below reserve; includes `lowest_human_grenades=<current>/<max>, threshold=<pct>` when debug logging is enabled; one-shot per bot+pickup defer episode |
...
| `smart-tag pickup hook skipped` | Smart-tag hook fired but did not become a BetterBots pickup/heal permission or pickup order; includes hook source, reason, template, target unit, pickup type, smart-tag target type, human-resolution result, and alternate tag mode |
...
| `reserved tagged health station for bot` | A human medicae smart-tag was converted into a per-bot explicit station reservation; this lets the selected bot keep the specific station after the short global tag window expires. |
| `health station destination refresh requested from human smart-tag` | Ping-only health-station use allowed a tagged or explicitly reserved medicae station and BetterBots requested an immediate vanilla destination refresh; normal health, human-priority, and Martyrdom guards still apply before this can fire. If the bot is already within its engine interaction-distance radius, BetterBots opens the interaction directly instead of refreshing the path again. |
| `health station interaction opened for bot` | BetterBots detected that the assigned bot passed medicae state checks and was within its engine interaction-distance radius, set the vanilla interaction unit, and snapped the stored level-unit destination to the bot position so `can_use_health_station` can enter the interaction node. |
| `health station heal applied: bot=<slot> ...` | `HealthStationInteraction.stop(result == "success")` completed on the server for a bot and BetterBots observed a health/corruption or charge delta. Includes precise current-health, permanent-damage, raw damage, raw permanent-damage, charge, and duration deltas. |
...
| `health station permitted: explicit human smart-tag order` | A bot kept using the specific medicae station originally assigned by a human smart-tag, even if the global tag window has expired. |
| `deferred health station to human player` | Healing deferral yielded a medicae station because a human was below reserve; includes `bot_health=<pct>, lowest_human_health=<pct>, threshold=<pct>` when debug logging is enabled. |
...
| `rescue interaction opened: target=<unit> need_type=<type> interaction=<type>` | BetterBots detected that the assigned rescuer can already interact within the engine interaction-distance radius, set the vanilla interaction unit, and snapped the ally-aid destination to the bot position so the direct rescue condition can enter the interaction node. |
| `rescue interaction succeeded: target=<unit> need_type=<type> interaction=<type>` | Direct rescue interaction completed successfully for `revive`, `remove_net`, `pull_up`, or `rescue`. This is stronger than `rescue interaction opened`. Key: `rescue_interaction_success:<bot>:<target>:<interaction>` (#108) |
| `rescue disabled state cleared: target=<unit> need_type=<type>` | A disabler rescue assignment (`pounced`, `mutant_charged`, `grabbed`, `consumed`, `warp_grabbed`) ended because the tracked ally no longer reports a disabled state. Key: `rescue_disabled_clear:<bot>:<target>` (#108) |
