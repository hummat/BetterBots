...
| `health station success: bot=<slot> health=<before>-><after>` | `HealthStationInteraction.stop(result == "success")` completed on the server for a bot. This is the authoritative medicae completion marker; it is stronger than `health station interaction opened for bot`. |
