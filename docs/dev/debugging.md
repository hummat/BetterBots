...
6. `/bb_scenarios`, `/bb_scenario <name> [distance] [count]`, and `/bb_scenario_clear`
   ...
   - `daemonhost_passive_near` spawns `chaos_daemonhost` with `optional_aggro_state = "passive"` and no forced target; use it for `#17` dormant-classifier validation. Decisive pass/fail markers are `movement safety steered away from daemonhost`, `skipped ping for chaos_daemonhost (reason: dormant_daemonhost)`, `melee/ranged suppressed (... daemonhost ... dormant=true)`, `ranged suppressed (target near dormant daemonhost)`, `blocked foreign weapon action ... daemonhost_avoidance ... dormant=true`, `daemonhost_nearby_target`, or an unexpected `ability allowed against daemonhost ... dormant=false`.
