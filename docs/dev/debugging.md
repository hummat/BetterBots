...
| `health station destination refresh requested from human smart-tag` | Ping-only health-station use allowed a tagged medicae station and BetterBots requested an immediate vanilla destination refresh; normal health, human-priority, and Martyrdom guards still apply before this can fire. |
