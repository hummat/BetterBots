...
- `queued ranged bash for <template> target=<breed> (bot=<slot>, fire_input=<input>)` (`#33` ranged-special identity; BetterBots rewrote close-range Ogryn heavy-stubber/thumper fire into a weapon bash for a valuable target)
