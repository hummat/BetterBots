...
- `grenade pickup skipped: ability does not use grenade pickups` (the equipped blitz replenishes through cooldown/passive logic instead of world grenade pickups, so grenade refill arbitration is bypassed)
- `grenade pickup skipped: cooldown-based blitz` (the bot has no grenade charges at all for this blitz path, so grenade refill logic is not applicable)
- `grenade pickup skipped: no ability extension` (grenade refill logic could not resolve the bot's `ability_system`, so reserve evaluation did not run)
...
- `bot <slot> skipped ping for <target> (reason: already_tagged|no_los|hold_last_tag|companion_tag)` (ping system — meaningful suppression, one-shot per repeated target/reason; `companion_tag` means an Arbites bot yielded enemy tagging to mastiff smart-tagging instead of issuing a normal ping on the same target)
...
- `bot <slot> skipped companion tag for <target> (reason: no_los)` (Arbites mastiff smart-tag — LOS suppression, one-shot per repeated target/reason)
