...
- `rescue interaction opened: target=<unit> need_type=<type> interaction=<type>` (`revive_ability.lua` detected that the assigned rescuer can already interact and snapped the ally-aid destination to the bot position so vanilla `can_revive` / `can_remove_net` / ledge / hogtied conditions can enter the interaction node)
