...
- `health station destination refresh requested from human smart-tag` (ping-only health-station mode allowed a tagged medicae station and requested an immediate vanilla destination refresh; normal health, human-priority, and Martyrdom guards still apply before this can fire)
