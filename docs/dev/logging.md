...
- `melee special prelude queued before <attack> (family=<family>)` (`#33`/`#103` melee-special identity; BetterBots prepended `special_action` before the chosen melee attack, with the family marker distinguishing powered weapons, chain weapons, power mauls, Ogryn latrine shovels, and Ogryn club uppercuts)
