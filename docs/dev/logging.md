...
- `grenade held <grenade> (bot=<slot|?>, rule=<rule>, nearby=<N>, distance=<d|none>, challenge=<N>, elites=<N>, specials=<N>, monsters=<N>, breed=<breed|none>, peril=<N|nil>, companion=<present|missing>, companion_pos=<present|missing>, target_pos=<present|missing>, companion_nearby=<N>, companion_challenge=<N>, companion_priority=<N>, companion_monsters=<N>)` (grenade/blitz heuristic withheld use for an actionable reason; `rule=*block_recent_use` is the confirmation signal for the non-explosive reuse pacing gate on fire/smoke-type grenades; `companion_*` fields are dog-centered Remote Detonation context)
...
- `fallback ability blocked: grenade sequence active for <grenade>` (the template fallback deferred a new combat ability until the grenade/blitz sequence releases its slot)
- `BT enter blocked <ability> (grenade sequence active for <grenade>)` (the vanilla BT ability path deferred a combat ability for the same slot-exclusion rule)
- `released weapon lock for forced slot_unarmed during <ability>` (an interaction or disruptive character state forced the bot unarmed, so BetterBots released its item/grenade lock before the engine emitted the state animation)
...
- `grenade pickup permitted: all eligible humans above reserve` / `grenade pickup permitted: human smart-tag order` (decision-level signal for grenade refills; BetterBots reserved the pickup for the bot through normal reserve arbitration or an explicit human smart-tag. Reserve-arbitrated lines include `lowest_human_grenades=<current>/<max>, threshold=<pct>` when debug logging is enabled. One-shot per bot+pickup reservation episode, not every tick.)
- `grenade pickup bound into ammo slot` / `grenade pickup bound into ammo slot from human smart-tag` (the chosen grenade pickup was attached to the ammo-pickup fields so vanilla interaction code can collect it. The smart-tag variant proves the explicit tag survived the later reserve-policy pass. One-shot per bot+pickup reservation episode, not every tick.)
...
- `released reserved grenade pickup after leaving range` (a sticky proactive BetterBots grenade reservation was dropped because the bot drifted beyond the pickup follow range before interaction completed; explicit smart-tag grenade orders keep their reservation)
- `grenade pickup deferred to human reserve` (the bot yielded a nearby grenade refill because at least one eligible human grenade user was below reserve. Includes `lowest_human_grenades=<current>/<max>, threshold=<pct>` when debug logging is enabled. One-shot per bot+pickup defer episode, not every tick.)
...
- `close-range ranged family kept ranged target type (family=<family>, distance=<d>, melee=<score>, ranged=<score>)` (`#41` narrow Sprint 3 override; a supported close-range ranged family kept the bot in ranged mode under point-blank pressure instead of flipping to melee)
- `close-range ranged target skipped (reason=immediate_melee_pressure, bot=<slot>, family=<family>, weapon=<template>, secondary_status=<status>, breed=<breed>, distance=<d>, chosen=<melee|ranged>, melee=<score>, ranged=<score>)` (`#41` close-pressure validation; a supported close-range ranged family was available but BetterBots let the bot return to melee because the target was inside the immediate-pressure cutoff)
...
- `animation guard fell back to plain anim_event for <variable> (<reason>)` (debug-only animation guard fallback; a bot-only animation variable lookup failed, so BetterBots emitted the plain animation event instead of the variable-float event; expected for missing bot animation variables and no longer counted as a DMF warning)
- `BetterBots: suppressed failed bot <anim_event|anim_event_1p> '<event>' (<error>)` (one-shot warning per method+event name; Stingray rejected the bot's `Unit.animation_event` call. Always investigate it. Errors outside that API call still raise.)
- `released grenade weapon lock for interaction during <grenade>` (debug confirmation that a pending or active interaction took ownership of the bot's wielded slot during a grenade sequence)
...
- `rescue interaction opened: target=<unit> need_type=<type> interaction=<type>` (`revive_ability.lua` detected that the assigned rescuer can already interact within the engine interaction-distance radius and snapped the ally-aid destination to the bot position so vanilla `can_revive` / `can_remove_net` / ledge / hogtied conditions can enter the interaction node)
- `rescue interaction succeeded: target=<unit> need_type=<type> interaction=<type>` (source-backed completion marker from the direct rescue interaction `stop(...)` hooks; emitted only for server-side `result == "success"` on `revive`, `remove_net`, `pull_up`, or `rescue`)
- `rescue disabled state cleared: target=<unit> need_type=<type>` (completion marker for disabler-target rescue assignments where no direct interaction exists; emitted when the tracked `pounced` / `mutant_charged` / `grabbed` / `consumed` / `warp_grabbed` ally leaves the disabled state)
...
- `reserved tagged health station for bot` (a medicae smart-tag was converted into a per-bot explicit station reservation, so the selected bot can keep the same station after the short global tag window expires)
- `deferred health station to human player` / `deferred medical crate to human player` (healing deferral yielded a medicae station or med-crate because a human player was below the configured reserve; station lines include `bot_health=<pct>, lowest_human_health=<pct>, threshold=<pct>` when debug logging is enabled)
...
- `health station destination refresh requested from human smart-tag` (ping-only health-station mode allowed a tagged or explicitly reserved medicae station and requested an immediate vanilla destination refresh; normal health, human-priority, and Martyrdom guards still apply before this can fire; skipped when the bot is already inside its engine interaction-distance radius)
- `health station interaction opened for bot` (BetterBots detected that the assigned bot passed medicae state checks and was inside its engine interaction-distance radius, set the vanilla interaction unit, and snapped the stored level-unit destination to the bot position so `can_use_health_station` can enter the interaction node)
- `health station heal applied: bot=<slot> ...` (server-side `HealthStationInteraction.stop(result == "success")` completed for a bot and BetterBots observed a health/corruption or charge delta; includes precise current-health, permanent-damage, raw damage, raw permanent-damage, charge, and duration deltas)
- `health station no-op: bot=<slot> ...` (the bot reached `HealthStationInteraction.stop(result == "success")`, but BetterBots observed no health/corruption or charge delta; this means the BT interaction completed but the station likely preserved its charge because the engine found nothing to heal)
- `health station permitted: explicit human smart-tag order` (a bot kept the specific medicae station assigned by a human smart-tag after the global tag window expired; this is the medicae counterpart to explicit grenade reservation persistence)
