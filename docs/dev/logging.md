...
- `health station success: bot=<slot> health=<before>-><after>` (server-side `HealthStationInteraction.stop(result == "success")` completed for a bot; this is the authoritative medicae completion marker and is stronger than `health station interaction opened for bot`)
