...
29a. Ranged weapon specials (#33 follow-up, via `ranged_special_action.lua` + `weapon_action.lua`):
    ...
    - rewrite policy is target-value driven: visible elite/special/captain/monster/boss or armored/super-armor target required; `shotgun_p2_m1`, flashlight variants, mauls, and generic bash/pistol-whip specials are still out of scope
    ...
    - rippergun bayonet and ranged-bash support are single-step: successful queueing logs `queued rippergun bayonet ...` or `queued ranged bash ...` with template, target breed, bot slot, and original fire input
