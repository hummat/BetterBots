...
22. Ammo, grenade, and mule pickup policy (#72 / #89 / #32 / #24 / #88, via `ammo_policy.lua` + `mule_pickup.lua` + `pocketable_pickup.lua`):
    ...
    - `smart_tag_orders.lua` hooks `SmartTagSystem.set_tag` to record all human unit-tag permissions, `set_contextual_unit_tag` for first-time item pings, and `trigger_tag_interaction` for already-tagged items, then routes explicit item-tag interactions back into the existing `BotOrder.pickup(...)` path instead of inventing a second order system; marker interactions that omit `target_unit` resolve it from the tag id before routing
