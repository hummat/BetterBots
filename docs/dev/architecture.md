...
13a. Rescue priority (#108, via `revive_ability.lua`):
    ...
    - when the assigned rescuer is already within `InteractorExtension:can_interact(...)` range, the hook sets `behavior.interaction_unit` and snaps `target_ally_aid_destination` to the bot position so the generated vanilla revive/remove-net/ledge/hogtied condition is not blocked by the strict 0.05m destination epsilon
