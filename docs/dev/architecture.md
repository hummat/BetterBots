...
11. Settings surface (`settings.lua`):
    ...
    - **Bot compensation settings**: `bot_survivability_profile` defaults to `auto`, which preserves vanilla `BotSpawning.get_bot_config_identifier()` difficulty scaling. Fixed `medium`/`high` profiles return those vanilla config identifiers directly. `none` preserves difficulty-based bot profile selection but suppresses the medium/high bot stat buff during spawn. `enable_bot_incoming_damage_reduction` defaults to true; disabling it temporarily clears `bot_power_level_modifier` on `minion_attack.lua` ranged/melee calls so bot targets take the unscaled player-target damage.
...
11b. Bot survivability compensation (via `bot_compensation.lua`):
    - hook `BotSpawning.get_bot_config_identifier`: optional fixed medium/high tier override for vanilla bot config selection and medium/high bot buffs
    - hook `GameModeCoopCompleteObjective.on_player_unit_spawn` and `GameModeExpedition.on_player_unit_spawn`: when survivability is `none`, return `low` only inside the bot spawn-buff check so the stat buff is skipped without forcing low-tier bot cosmetics/loadouts
...
13a. Rescue priority (#108, via `revive_ability.lua`):
    - hook `BotBehaviorExtension._verify_target_ally_aid_destination` (post-process): before BT evaluation and movement refresh, detects rescue-critical allies on the bot side and assigns the nearest available bot, with a short per-target owner lease so close bots do not churn the assignment every frame; if the current owner drifts more than 3m worse than another available bot, ownership is released so the closer bot can take over
    - direct interaction rescues use the vanilla ally-aid seam: `perception.target_ally`, `target_ally_needs_aid`, `target_ally_need_type` (`knocked_down`, `netted`, `ledge`, `hogtied`), `behavior.revive_with_urgent_target = true`, `perception.force_aid = true`, and `follow.needs_destination_refresh = true`; stale combat targets are cleared while the rescue assignment is active so the generated tree does not fall back into ordinary priority-target combat before reaching the ally
    - current limitation: Darktide excludes hogtied units from `side.valid_player_units`, and BetterBots' selector reads the same collection; the hogtied seam exists but respawned allies cannot currently enter it
    - when the assigned rescuer passes `InteractorExtension:can_interact(...)` and is within the engine interaction-distance radius, the hook sets `behavior.interaction_unit` and snaps `target_ally_aid_destination` to the bot position so the generated vanilla revive/remove-net/ledge/hogtied condition is not blocked by the strict 0.05m destination epsilon
    ...
    - source-backed rescue completion diagnostics hook the direct rescue interaction `stop(...)` methods (`revive_interaction`, `remove_net_interaction`, `pull_up_interaction`, `rescue_interaction`) and log `rescue interaction succeeded` only for server-side `result == "success"`; disabler-target rescues log `rescue disabled state cleared` when the tracked ally leaves the disabled state
...
21. Healing deferral (#39, via `healing_deferral.lua`):
    ...
    - when every eligible human is above reserve, the same hook now promotes health-station demand for any damaged non-Martyrdom bot, overriding vanilla's heavier-damage threshold so slightly-damaged or corruption-only bots stop leaving spare medicae charges stranded
    - scarce station charges are reserved for clearly more-injured bots first; a healthier bot can still use medicae when spare charges remain after worse bots are covered
...
22. Ammo, grenade, and mule pickup policy (#72 / #89 / #32 / #24 / #88, via `ammo_policy.lua` + `mule_pickup.lua` + `pocketable_pickup.lua`):
    ...
    - post-processes vanilla `_update_mule_pickups()` assignment to ignore the stock "suppress mule pickup while any human is within ~20m of the book" rule; BetterBots now materializes valid explicit pickup orders into `pickup_component.mule_pickup`, claims the nearest visible eligible unassigned book for a bot with a free mule slot and an in-leash follow position, then marks destination refresh so ordinary book pickup can happen in the common nearby-player case
    ...
    - `update_dispatcher.lua` runs the carried pocketable state machine once the bot already owns the item: combat stims self-use on high-threat entry, the corruption-heal syringe self-uses conservatively when the bot itself needs healing/corruption relief and combat is calm, and ammo/medical crates auto-deploy when at least two allies are in coherency, no enemy is currently engaged, and the team actually needs the resource; explicit `Need Ammo` / `Need Health` wheel requests bypass those conservative auto-deploy gates for matching carried crates
    - `smart_tag_orders.lua` hooks direct `SmartTagSystem.set_tag`, `set_contextual_unit_tag` first-time item pings, and `trigger_tag_interaction` already-tagged item interactions, then routes explicit item tags back into the existing pickup lanes instead of inventing a second order system: normal pickups use `BotOrder.pickup(...)`, while `small_grenade` tags reserve the target through `ammo_policy.lua`'s grenade-refill path; nested vanilla `set_tag` calls are suppressed so one player tag creates at most one bot pickup order, and marker interactions that omit `target_unit` resolve it from the tag id before routing
    ...
    - the routing layer reuses `MulePickup.should_block_pickup_order(...)` so existing BetterBots policy gates still apply (unsupported pocketables, disabled books, human-slot-open pocketables), selects the nearest eligible live bot on the interactor's side, and is guarded by the `enable_smart_tag_orders` setting, local SoloPlay detection (`SoloPlay.is_soloplay()` / `Managers.multiplayer_session:host_type()` before the older `host_singleplay` flag), plus a class-table hot-reload sentinel on `SmartTagSystem`
    - strict pickup mode (`pickup_require_tag`, default off) records human smart-tags independently of explicit order routing, then blocks proactive ammo, grenade-refill, book, crate, and stim claims unless the target pickup was recently tagged; explicit smart-tag pickup orders still bypass this strict proactive gate through the normal `BotOrder.pickup(...)` order state
22b. Communication-wheel response (#56, via `com_wheel_response.lua` + `settings.lua` + resource policy hooks):
    ...
    - MVP scope is intentionally narrow: `com_cheer` temporarily overrides the resolved behavior preset to `aggressive`, while `com_need_ammo` and `com_need_health` set short-lived human request flags that `ammo_policy.lua` and `healing_deferral.lua` treat as stronger-than-reserve deferral signals; `pocketable_pickup.lua` also treats them as immediate deploy commands for matching carried ammo/medical crates
...
24a. Movement safety / hazard avoidance (#107, via `hazard_avoidance.lua` and `sprint.lua`):
    ...
    - applies a conservative ledge endpoint guard after vanilla movement: projected short movement that fails nav continuity or drops too far cancels pending dodge only. Ordinary movement fails open because the coarse nav/drop probe misclassifies stairs and downhill traversal.
    - daemonhost keepout steering remains in `sprint.lua` because it reuses the per-frame non-aggroed daemonhost scan; bots inside a tight movement radius get a distance-weighted movement bias away from the daemonhost, bots inside `daemonhost_keepout_distance` suppress risky actions, and sprint suppression still uses the wider 20m anti-aggro radius. The scan uses the bot side's known enemy relation list plus `Managers.state.minion_spawn:spawned_minions()`, not just active `ai_target_units`, so passive daemonhosts can still be treated as known hazards before they become bot AI targets.
...
29. Sustained-fire hold bridge (#87, via `sustained_fire.lua`):
    ...
    - also injects vanilla-style `aim_at_node = { "j_head", "j_spine" }` for allowlisted weakspot families (lasguns, autoguns, bolters/bolt pistols, stub revolvers, plasma guns, and both twin-linked `ogryn_heavystubber_p1_*` and single `ogryn_heavystubber_p2_*` heavy stubbers) when the template leaves `aim_at_node` unset (#91 MVP)
...
29a. Ranged weapon specials (#33 follow-up, via `ranged_special_action.lua` + `weapon_action.lua`):
    ...
    - rewrites queued close-range rippergun fire into `stab` for `ogryn_rippergun_p1_m1/m2/m3` when the current target is inside the configured bayonet distance and is elite/special/captain/monster/boss or armored/super-armor; target-type hysteresis still lets close hard-armored melee targets flip to melee instead of trapping the bot in bayonet spam
...
31b. Charge/dash nav validation (#13, via `charge_nav_validation.lua`):
    ...
    - rejects launch endpoints inside the dormant daemonhost keepout zone before the GwNav ray query (`charge_nav=daemonhost_target_near`)
...
**Rule 2: production `hook_require` registrations use `mod:hook_require_now`.** `mod:hook_require_now(path, callback)` registers the DMF `hook_require` callback and immediately replays the same installer when `package.loaded[path]` already has the engine module. It must not cold-`require()` the target, because DMF can execute the registered callback during that load and installer failures must not be swallowed as missing modules. Cached replay only accepts table modules; installer failures are warned and rethrown so cold-load and replay failures are visible in the console. This closes the load-order hole where another mod requires a shared engine path before BetterBots boots. The only bare `mod:hook_require` exception is diagnostic-only `BtRandomUtilityNode` combat utility logging; losing it cannot drop gameplay patches or crash guards.

Leaf modules that can be unit-tested without the full BetterBots bootstrap should call a local `_hook_require_now(...)` helper that uses `_mod.hook_require_now` in production and falls back to `_mod["hook_require"]` for narrow test doubles. The helper must pass caller level `4` to preserve the real registration callsite and must warn before using the fallback. Do not add new production `mod:hook_require(...)` calls.
...
Mitigation: `_evaluate_grenade_heuristic` accepts `opts.revalidation = true`. When set, the dispatcher temporarily mutates `context.num_nearby = num_nearby + 1` for the duration of the call and restores the original value before returning. `grenade_fallback.lua` passes `{ revalidation = true }` only at the aim-window re-check. The relaxation is bounded to one enemy -- `0 -> 1` is still held by the heuristic itself. After the bot has already committed to an aimed throw, `grenade_fallback` treats a revalidation rule ending in `_hold` as a soft hold and continues the throw, while hard block rules such as `*_block_melee_range` still abort.
...
Analysis via `bb-log events [summary|rules|holds|items|scenarios|trace|raw]`. See `docs/dev/logging.md` for event schema.
