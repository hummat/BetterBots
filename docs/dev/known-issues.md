...
The v1.2.3 animation-slot fix still needs the mandatory cold-boot checks in both mod load orders. Two 2026-07-25 AirGPU runs with a Rock + Point-Blank Barrage Ogryn confirmed the stance/grenade half live (`grenade blocked: combat ability active` and `fallback ability blocked: grenade sequence active`, zero crash signatures), both with BetterBots first in the load order. The forced `slot_unarmed` release and the BT-enter gate have unit coverage only, and no run has used the BetterBots-last order. See `docs/dev/validation-tracker.md`.

## High severity

1. ~~Ogryn animation-event crashes when grenade use overlaps a combat ability or disruptive state~~ **Fixed for v1.2.3; in-game validation pending.** Two Nexus logs showed invalid events on Ogryn grenade state machines: `airtime_bwd` after a Poxburster catapulted a Rock-using bot, and `mid_reload_finished` when Gunlugger stance overlapped the grenade slot. BetterBots kept the grenade slot locked while the engine tried to force `slot_unarmed` or enter the combat ability, so the next event reached the wrong animation state machine. The fix releases the lock for forced unarmed transitions and prevents combat abilities and grenade sequences from entering the same input-parser window. The grenade lock also yields to active interactions, including the four templates that wield `slot_device`. As a final guard, a failed Stingray `Unit.animation_event` call on a bot produces a one-shot warning instead of ending the session; errors outside that API call still raise.
