...
| 90 | Melee/ranged target type hysteresis | Implemented, but still validation-gated. `target_type_hysteresis.lua` still handles the math layer by wrapping `bot_target_selection_template.bot_default`, recomputing vanilla melee/ranged scores, and only flipping `target_enemy_type` when the opposite mode clears both a `0.10 * max(abs(scores), 1)` margin and a small current-type momentum bonus. After a 2026-04-15 log proved real symptom churn (`wrong slot ...` / `switch_melee|switch_ranged entered`) with zero `type flip` / `type hold` lines, `condition_patch.lua` also added a BT-side opposite-type reswitch debounce for non-priority targets. Runtime verification now needs both the new debounce log (`suppressed opposite-type switch ...`) and a visible drop in switch churn before closure. |
