...
### v1.1.0 (2026-05-02)
- Post-1.0 hardening: daemonhost avoidance hardened (#17), hazard movement safety (#107), pickup follow-through fixed, healing priority fixed, rescue priority (#108), non-veteran bot profile refresh, scenario harness expanded (#100), core regression checklist, perf cap verified (#106).
...
| 91 | Bot weakspot aim MVP | Closed 2026-04-13; expanded 2026-04-29. `ranged_meta_data.lua` injects vanilla-style `aim_at_node = { 'j_head', 'j_spine' }` for lasguns, autoguns, bolters/bolt pistols, stub revolvers, plasma guns, and both twin-linked `ogryn_heavystubber_p1_*` and single `ogryn_heavystubber_p2_*` heavy stubbers when the template leaves `attack_meta_data.aim_at_node` unset. Existing `aim_at_node` values still win, and vanilla `target_breed.override_bot_target_node` behavior is unchanged. Live runtime confirmation now exists via `weakspot aim selected j_head|j_spine ...`. Per-breed weakspot map (Mauler/Crusher/Bulwark) remains deferred to `#92`. |
...
| 33 (narrow) | Activate_special melee | **Code-complete 2026-04-18; follow-ups landed 2026-04-19, 2026-04-20, 2026-04-22, and 2026-04-29; chain-family follow-up `#103` closed 2026-04-22 after live validation.** `melee_attack_choice.lua` now caches supported weapon-special metadata during `BtBotMeleeAction.enter` and prepends `special_action` before the chosen attack when the current family-specific rule says the target is worth it. The first post-1.0 pass added chain-family `toggle_special` weapons (`chainaxe_`, `chainsword_`, `chainsword_2h_`) for elite/captain/monster/boss or super-armor targets. The next pass split the old powered bucket into real families: 1H power swords now arm in live combat windows instead of only elite/special cases, Zealot 2H power swords resolve their `toggle_special` path correctly, force swords stay targeted, thunder hammers now widen to armored/heavy elites plus captain/monster/boss, Ogryn latrine shovels (`ogryn_club_p1_m2/m3`) now fold for high-health or armored targets with heavy follow-up bias, and direct Ogryn club/pickaxe/combat blade specials are paced after a queued `special_action` so they do not replace every melee swing. First-pass ranged specials also landed for supported shotgun special-shell loaders; broader bash/bayonet ranged-special policy remains post-1.0. |
...
| 24 (a) | Medicae discipline | **Code-complete 2026-04-18; in-game validation pending.** `healing_deferral.lua` owns the human-first medicae policy, Martyrdom carve-out, and station/deployable deferral hooks. The original Sprint 4 MVP shipped conservative bot-side onset rules here; `#97` later narrowed that to "any missing amount once humans are above reserve" for non-Martyrdom bots without reviving the broad wound-cure / ally-handoff scope. A 2026-05-02 follow-up added station-charge-aware bot ordering so scarce charges go to clearly more-injured bots first. A later setting can require a human smart-tag before bots use health stations at all. Broad ally-handoff behavior is still not shipped. |
...
| 88 | Deployable crate carry + deploy | **Code-complete 2026-04-18; in-game validation pending.** Ammo + medical crates now ride the vanilla mule carry path and auto-deploy from `slot_pocketable` only when the bot is safe, at least two allies are in coherency, and the team actually needs the resource; explicit `Need Ammo` / `Need Health` wheel requests make a bot deploy the matching carried crate immediately. |

#### Sprint 5 — Team coordination + safety

| # | Issue | Notes |
|---|-------|-------|
| 56 | Communication wheel response | **Code-complete 2026-04-18; in-game validation pending.** Narrow Solo Play MVP in `com_wheel_response.lua`: hook `Vo.on_demand_vo_event` so vanilla + ForTheEmperor wheel events share one detection path, temporarily force `aggressive` after `com_cheer`, treat `com_need_ammo` / `com_need_health` as short-lived human-priority overrides in `ammo_policy.lua` / `healing_deferral.lua`, and use those same requests as immediate deploy commands for matching carried crates in `pocketable_pickup.lua`. `Need Help` convergence and location-directed movement stay out of scope because BetterBots still has no movement-order layer. |
| 96 | Smart-tag item interaction bridge | **Closed 2026-05-02; direct `set_tag` pickup routing repaired 2026-05-15.** `smart_tag_orders.lua` hooks direct `SmartTagSystem.set_tag`, `set_contextual_unit_tag` first-time item pings, and `trigger_tag_interaction` already-tagged item interactions; marker interactions that omit `target_unit` resolve it from `tag_by_id(...)` before routing. Explicit item-tag interactions for ammo, books, and supported pocketables route back into `BotOrder.pickup(...)`; world grenade-refill tags reserve the target through `ammo_policy.lua`'s grenade lane. Existing `MulePickup`/`PocketablePickup` policy gates still apply where relevant, and the bridge chooses the nearest eligible bot. Live proof: repeated `smart-tag pickup routed ... to bot ... (family=slot_order)` lines in `console-2026-05-02-14.33.58...`; the grenade tag path is unit-covered and pending live validation. Health-station tags are recorded only as optional healing permission; location-style interactions stay out of scope for the MVP. |
| 97 | Non-book resource arbitration | **Partially validated 2026-05-03 from May 2-3 logs; still open.** Ammo top-off is confirmed at tiny deficits (`98%->102%`, `98%->110%`, `96%->100%`, `93%->100%`), and world grenade pickup is confirmed via `grenade pickup success: small_grenade` at `2->4/4`, `2->3/3`, `1->4/4`, and `3->4/4`. Medicae is practically confirmed by `health station permitted: humans above reserve and bot not full` followed by `shield (health_station)` and then `deferred health station because bot is already full`. Remaining gaps: med-crate / health-deployable use completion was not observed, and no log gives a clean ammo-driven grenade-refill-via-ammo-pickup marker. |
...
| # | Issue | Notes |
|---|-------|-------|
| 98 | Sparse metadata hardening | **Closed 2026-05-03.** Targeted boundary guards landed in `melee_attack_choice.lua`, `melee_meta_data.lua`, and `ranged_meta_data.lua`: truthy non-table `attack_meta_data` is now treated as invalid at the boundary instead of being indexed as if it were sane, while the metadata injectors replace malformed values only for the enabled lifetime of the feature and restore the original value on disable. Regression coverage includes non-table top-level melee selection plus malformed existing `attack_meta_data` on both melee and ranged injectors. Closed from focused spec evidence (`159 successes, 0 failures`) because malformed-template hardening is not meaningfully validated by waiting for a random in-game repro. No blanket `pcall`, no broad refactor. |
...
|---|---|---|---|
| R1 | `#108` Rescue priority | Closed | Closed from `console-2026-04-29-17.10.48...`: repeated bot 2 `human revive priority assigned` markers on the same human target, ally-rescue sprint, `shield (revive)`, and follow-up `interacting with` blocks confirm the BetterBots assignment and vanilla interaction seam. Later regression specs extend the forced path to netted bot allies and source-verified disabler states (`pounced`, `mutant_charged`, `grabbed`, `consumed`, `warp_grabbed`) while keeping humans ahead of bots. Completion diagnostics now include source-backed `rescue interaction succeeded` markers for direct rescue interactions and `rescue disabled state cleared` for disabler rescues. Respawned/hogtied allies remain outside this path because Darktide removes them from `valid_player_units` before BetterBots selects a rescue target. |
| R2 | `#17` Daemonhost avoidance | Closed | Closed from `console-2026-05-02-14.33.58...`: passive/awake-but-not-aggroed daemonhost detection, close movement steering, mixed-target ranged suppression, and grenade/blitz/ability holds all fired in the targeted scenario. |
| R3 | `#106` Perf cap miss | Closed | Closed from April 29 `bb-perf:auto` totals: median `95.7 us/bot/frame`, worst `116.5`, and `ability_queue + grenade_fallback` below 50% in checked runs. No second perf slice is justified until fresh logs exceed the bar again. |
| R4 | `#100` Scenario validation harness | Closed | MVP closed from April 29 live scenario rows: three `mauler_weakspot` runs spawned 10 `renegade_executor` units at distance 22 and emitted `scenario_result status=spawned`. First library expansion adds `mixed_horde_pressure`, `daemonhost_passive_near`, and `daemonhost_aggroed_control`; future work should add scenario result predicates, not more ad-hoc spawn commands. |
| R5 | `#96` Smart-tag item bridge | Closed | Closed from `console-2026-05-02-14.33.58...`: explicit supported item tags produced repeated `smart-tag pickup routed ... to bot ... (family=slot_order)` lines after the liveness fix. |
...
| R7 | `#92` Per-breed weakspot aim | Deprioritize | Mauler scenario validated the player-visible anti-armor behavior and spine override. Do not chase generic weakspot proof manually; revisit only if a cheap scenario can expose it. |
...
| R10 | `#107` Barrel avoidance | Closed | Closed from `console-2026-05-02-14.33.58...`: `hazard_prop triggered`, `aoe_threat accepted`, `aoe_threat consumed`, and `movement safety blocked` markers proved the hazard/movement path. |

Lower ROI for now: `#80` and `#22` are architectural follow-ups; `#86` is cool but less important than revive priority work already closed under `#108`; broad `#24`, `#28`, and `#84` carry support and UX burden; `#8` is DLC-gated.
