...
| # | Issue | Notes |
|---|-------|-------|
| 14 | Ability cooldown staggering | **Fix landed (`d890695`), needs revalidation.** Two integration bugs found 2026-04-11 in-game: `TeamCooldown.record()` was called with the variant talent name (`psyker_discharge_shout_improved`) instead of the base/semantic key (`psyker_shout`) — silent no-op for variants. Fallback queue path (`ability_queue.lua`) was missing the `is_suppressed` check entirely, while ~99% of solo-play activations come through fallback. Both fixed. Re-run with vet VoC + psyker Venting Shriek lineup to confirm `team_cd:` suppression entries fire on close shout pairs. |
