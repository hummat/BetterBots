...
| 89 | Bot grenade pickup heuristic | Implemented. `ammo_policy.lua` now piggybacks the vanilla `pickup_component.ammo_pickup` slot for nearby `small_grenade` refills whenever every eligible human is above the configured grenade reserve threshold. Cooldown-only blitz users are ignored, and deferred grenade pickups do not block bots from still taking ammo. The ammo-pickup regression is fixed and live `grenade pickup success` logs now exist, but they are still tied to `small_clip` / `ammo_cache_deployable` restocks rather than an unambiguous standalone `small_grenade` world pickup, so the issue stays open. |
