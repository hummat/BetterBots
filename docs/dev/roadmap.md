...
| 90 | Melee/ranged target type hysteresis | Vanilla `bot_target_selection_template.lua:73-82` picks `target_enemy_type` via hard `>` on melee vs ranged scores — no margin, no momentum bias, re-evaluated every 0.3 s. Close scores produce 3 Hz weapon-swap thrash (`switch_melee` ↔ `switch_ranged`), most visible on Veteran melee+lasgun against grouped enemies at 8–14 m. Proposed fix: hook perception to require `margin = 0.10 * max(scores)` for type flips and grant currently-active type a small momentum bonus. Local to perception, no BT changes. Pairs naturally with #41 + #87 in the v0.11.0 ranged-quality cluster — single in-game validation session covers all three. Filed 2026-04-11 after CLAUDE-Opus audit of `bot_target_selection_template.lua`. |
| 91 | Bot weakspot aim MVP | Vanilla bots aim `j_spine` (chest) for every shot — finesse stats and weakspot blessings/perks/talents are wasted. Only `high_bot_lasgun_killshot.lua:738` uses head aim in the entire game (`aim_at_node = { 'j_head', 'j_spine' }`, randomized 50/50). MVP: inject the same table into `attack_meta_data.aim_at_node` via `ranged_meta_data.lua` for a finesse-weapon allowlist (lasguns, autoguns, revolver, bolter). ~30 LoC mod + 40 LoC tests. Vanilla `target_breed.override_bot_target_node` chain unchanged → pox hounds/monstrosities still take body shots. Couples with #44 human-likeness for one-session validation. Filed 2026-04-11. Per-breed weakspot map (Mauler/Crusher/Bulwark) deferred to #92. |
| 93 | Grenade ballistic arc fix | `grenade_fallback._set_bot_aim` aims flat-horizontal at target's `POSITION_LOOKUP` (feet) with zero gravity compensation and zero target velocity lead. All grenade projectiles use `manual_physics` with `gravity = 12.5–17.5` and `speed_initial = 15–30 m/s` — at 25 m a flat throw drops ~6 m short. Vanilla shoot action already solves this via `Trajectory.angle_to_hit_moving_target` (`bt_bot_shoot_action._wanted_aim_rotation:304-323`). Fix: copy the solver into `_set_bot_aim`, switch to `set_aim_rotation`. Affects veteran/ogryn/zealot/arbites grenades + zealot throwing knives (gravity 17.5); smite/chain/assail/psyker knives (true_flight)/whistle/mines unaffected. Independent of #62 (which fixed facing direction). Filed 2026-04-11. |
