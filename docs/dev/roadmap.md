...
|---|---|---|---|
| R1 | `#108` Human revive priority | Needs testing | Code-complete 2026-04-28, refined 2026-04-29 after live churn evidence: `revive_ability.lua` now assigns a knocked-down human to a live bot at the pre-BT destination-verification seam, holds that owner briefly to avoid close-range assignment churn, forces `revive_with_urgent_target`, and refreshes movement so vanilla ally-aid pathing outranks urgent/priority enemy pathing. Needs another cold-boot Solo Play validation pass with debug logs. |
