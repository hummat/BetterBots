...
| R2 | `#17` Daemonhost avoidance | Validate/fix | Rage-quit class failure. Decisive first-action `stage`/`aggro_state` text logging was added on 2026-04-28; `daemonhost_passive_near` and `daemonhost_aggroed_control` are now the targeted validation pair. Run both before spending more time on random daemonhost mission logs. |
