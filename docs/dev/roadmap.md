...
| 44 | Human-likeness tuning (Tier A) | Implemented on branch. `human_likeness.lua` patches opportunity-target reaction times from `10-20` to `2-5`, `ability_queue.lua` adds `0.3-1.5s` combat-ability jitter for non-emergency fallback casts, and `engagement_leash.lua` now shrinks melee chase range under high `challenge_rating_sum` pressure. Grenade/blitz timing, aim tuning, and dodge tuning remain out of scope. |
