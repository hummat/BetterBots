...
*Theme: small, high-ROI polish. Trivial wins and one teammate-feel feature. The code is in `dev/v0.11.0` and the quality gate is green, but the release still waits on the remaining live-validation issues.*

| # | Issue | Notes |
|---|-------|-------|
| 32 | Mule item pickup | Implemented. `mule_pickup.lua` activates vanilla book-carry flow by mirroring `inventory_slot_name -> slot_name` and setting `bots_mule_pickup` on the side-mission book pickup templates. Tomes/scriptures are always enabled; grimoires are gated behind a new BetterBots toggle that defaults off and also blocks grimoire pickup orders while disabled. Crashy stale-unit cleanup was fixed on 2026-04-13, but live tome/scripture pickup confirmation is still missing, so the issue stays open. |
...
| 82 | Perf low-hanging fruit audit | Implemented. Final low-risk pass only: `ability_queue.lua` now caches one-shot `AbilityTemplates` injection instead of requiring/injecting on every fallback tick, and `target_selection.lua` now memoizes same-frame smart-tag, companion-pin, and slot-ammo lookups inside the hot per-target scoring path. `make check` is green at 943 tests, but there is still no fresh `/bb_perf` capture on the current branch, so the issue stays open. |
| 87 | Sustained fire for flamers and held-fire weapons | Implemented. `sustained_fire.lua` now observes queued `weapon_action` inputs, arms per-unit sustained state for supported full-auto / stream paths, and injects required raw hold inputs through `BotUnitInput._update_actions` until the path goes stale or clears. Covered: flamer, Purgatus, recon lasguns, autoguns, autopistol, dual autopistols, bolter hipfire, heavy stubbers, and rippergun braced fire. 2026-04-13 validation proved the parser/runtime cleanup and the correct stream inputs (`shoot_braced`, `trigger_charge_flame`), but still did not produce `holding sustained fire inputs`, so the issue stays open. Scope remains execution-only; ADS/brace choice still belongs to #41. |
| 89 | Bot grenade pickup heuristic | Implemented. `ammo_policy.lua` now piggybacks the vanilla `pickup_component.ammo_pickup` slot for nearby `small_grenade` refills whenever every eligible human is above the configured grenade reserve threshold. Cooldown-only blitz users are ignored, and deferred grenade pickups do not block bots from still taking ammo. The ammo-pickup regression is fixed and `ammo pickup success` is now logged, but no run has yet produced `grenade pickup success`, so the issue stays open. |
| 90 | Melee/ranged target type hysteresis | Implemented. `target_type_hysteresis.lua` wraps perception-layer `bot_target_selection_template.bot_default`, recomputes vanilla melee/ranged scores, and only flips `target_enemy_type` when the opposite mode clears both a `0.10 * max(abs(scores), 1)` margin and a small current-type momentum bonus. BT remains untouched; scope is strictly target-type stabilization. Runtime verification now uses `type flip ...` and `type hold ... over raw ...` messages, but a longer combat session is still needed before closure. |
| 91 | Bot weakspot aim MVP | Implemented. `ranged_meta_data.lua` now injects vanilla-style `aim_at_node = { 'j_head', 'j_spine' }` for lasguns, autoguns, bolters, and stub revolvers when the template leaves `attack_meta_data.aim_at_node` unset. Existing `aim_at_node` values still win, and vanilla `target_breed.override_bot_target_node` behavior is unchanged. Runtime verification now uses `weakspot aim selected j_head|j_spine ...`, but no live run has captured that signal yet, so the issue stays open. Per-breed weakspot map (Mauler/Crusher/Bulwark) remains deferred to `#92`. |
