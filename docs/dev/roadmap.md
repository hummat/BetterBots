...
| 96 | Smart-tag item interaction bridge | **Closed 2026-05-02.** `smart_tag_orders.lua` hooks `SmartTagSystem.set_tag` to record human tag permissions for strict pickup/medicae modes, `set_contextual_unit_tag` for first-time item pings, and `trigger_tag_interaction` for already-tagged items; marker interactions that omit `target_unit` resolve it from `tag_by_id(...)` before routing. Explicit item-tag interactions for ammo, world grenade refills, books, and supported pocketables route back into `BotOrder.pickup(...)`, reusing existing `MulePickup`/`PocketablePickup` policy gates and choosing the nearest eligible bot. Live proof: repeated `smart-tag pickup routed ... to bot ... (family=slot_order)` lines in `console-2026-05-02-14.33.58...`. Health-station tags are recorded only as optional healing permission; location-style interactions stay out of scope for the MVP. |
