...
| 90 | Melee/ranged target type hysteresis | Implemented, but still validation-gated. `target_type_hysteresis.lua` handles the math layer by wrapping `bot_target_selection_template.bot_default`, recomputing vanilla melee/ranged scores, and only flipping `target_enemy_type` when the opposite mode clears both a `0.10 * max(abs(scores), 1)` margin and a small current-type momentum bonus. After the 2026-04-15 live run proved real symptom churn (`wrong slot ...` / `switch_melee|switch_ranged entered`) with zero `type flip` / `type hold` lines, `condition_patch.lua` added a BT-side opposite-type reswitch debounce for non-priority targets, and 2026-04-16 follow-up debugging identified the missing math-layer signal: the target-selection template could already be loaded before BetterBots registered its `hook_require`, so the perception-layer wrapper never installed. The current branch now eagerly patches already-loaded target-selection templates (with a regression test for that startup order), but the issue stays open until a fresh live run shows `type hold` / `type flip` and the `target_type_hysteresis.post_process` perf row in mission logs. |
