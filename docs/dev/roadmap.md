...
| 13 | Navmesh charge/dash validation | **Implemented 2026-04-18.** Shared `charge_nav_validation.lua` validates the current `navigation_extension:destination()` with `NavQueries.ray_can_go(...)` before committing `ogryn_charge`, `adamant_charge`, and zealot dash variants. Wired into both `BtBotActivateAbilityAction.enter` and `ability_queue.lua`; same-destination failures cache for 0.5s so GwNav queries do not repeat every tick. Existing rescue-aim path remains complementary, not a substitute. |
