...
**Architecture:** Bottom-up implementation: settings infrastructure first (category tables, feature gates, preset resolver), then heuristic threshold tables + signature refactor, then UI (widget definitions + localization), then module gate wiring, then docs. Each layer builds on the previous and can be tested independently. The `balanced` preset produces identical behavior to current hardcoded values — zero regression for existing users.
...
Task 2 (heuristic signature refactor + threshold tables)
  ↓ hard — tests exercise the new (context, thresholds) signature
...
| 2 (heuristic thresholds) | 1 | `heuristics.lua`, `heuristics_spec.lua` |
...
## Chunk 2: Heuristic Threshold Tables & Signature Refactor

### Task 2: Add per-heuristic threshold tables and refactor to `(context, thresholds)` signature
...
This is the largest single change. Every `_can_activate_*` function currently takes either `(context)` or the 9-arg dispatch signature. All need to change to `(context, thresholds)` (or `(context, thresholds, veteran_extras)` for veteran_combat_ability).

**Key constraint:** The `balanced` threshold values MUST exactly match the current hardcoded constants. This ensures zero behavior change for existing users and that all 418 existing tests pass without logic changes (only mechanical signature updates).
...
3. Add threshold tables above each function (16 combat + 3 item heuristics)
4. Change each function signature from `(context)` to `(context, thresholds)` and replace hardcoded values with `thresholds.X`
5. Update `TEMPLATE_HEURISTICS` dispatch table to use new `(context, thresholds)` calling convention
6. Update `_evaluate_template_heuristic` to look up threshold tables and pass them
7. Update `evaluate_heuristic`, `evaluate_item_heuristic`, `evaluate_grenade_heuristic` to accept `opts.preset`
8. Update `_can_activate_veteran_combat_ability` to `(context, thresholds, veteran_extras)`
...
- [ ] **Step 2: Add threshold tables for all 16 combat heuristic functions**

Place each table directly above its corresponding function. The `balanced` values MUST match the current hardcoded constants exactly. Here are the tables (aggressive relaxes by ~30-50%, conservative tightens by ~30-50%):

**Example — `_can_activate_veteran_stealth`** (current hardcoded: `toughness<0.25 +nearby>=2`, `health<0.40 +nearby>=1`, `nearby>=5 +toughness<0.50`):
...
Do this for ALL 16 functions listed in the spec's "Gets threshold table" section. For each one:
- Read the current hardcoded values in the function
- Those become the `balanced` row
- Aggressive: relax numeric thresholds ~30-50% toward "easier to trigger"
- Conservative: tighten ~30-50% toward "harder to trigger"

Functions without threshold tables (broker_focus, broker_rage, stimm_field) keep `(context)` signature and receive `nil` for thresholds.

Also add 3 item heuristic threshold tables for zealot_relic, force_field, and drone:

```lua
local ZEALOT_RELIC_THRESHOLDS = {
    aggressive   = { team_toughness = 0.50, self_critical = 0.35, overwhelmed_max = 3 },
    balanced     = { team_toughness = 0.40, self_critical = 0.25, overwhelmed_max = 2 },
    conservative = { team_toughness = 0.30, self_critical = 0.15, overwhelmed_max = 2 },
}
-- (similar for FORCE_FIELD_THRESHOLDS and DRONE_THRESHOLDS)
```

- [ ] **Step 3: Refactor each function signature to `(context, thresholds)`**

For each of the 16 threshold-equipped combat heuristics, change the signature and replace hardcoded values with threshold lookups. Example for `_can_activate_veteran_stealth`:
...
For `_can_activate_veteran_combat_ability`, the new signature is:
```lua
local function _can_activate_veteran_combat_ability(context, thresholds, veteran_extras)
```
Where `veteran_extras = { conditions, unit, blackboard, scratchpad, condition_args, action_data, is_running, ability_extension }`. The vanilla VoC fallback reads from `veteran_extras`:
```lua
local can_activate_vanilla = veteran_extras.conditions._can_activate_veteran_ranger_ability(
    veteran_extras.unit, veteran_extras.blackboard, veteran_extras.scratchpad,
    veteran_extras.condition_args, veteran_extras.action_data, veteran_extras.is_running
)
```

The VoC branch also gets a threshold table (`VETERAN_VOC_THRESHOLDS`) and the stance branch gets its own (`VETERAN_STANCE_THRESHOLDS`).

- [ ] **Step 4: Update `_evaluate_template_heuristic` to look up and pass thresholds**

```lua
-- Add a master lookup table:
local HEURISTIC_THRESHOLDS = {
    veteran_stealth_combat_ability = VETERAN_STEALTH_THRESHOLDS,
    zealot_dash = ZEALOT_DASH_THRESHOLDS,
    -- ... (all 16 tables, keyed by template name)
    -- zealot_targeted_dash* variants share ZEALOT_DASH_THRESHOLDS
}

local function _evaluate_template_heuristic(
    ability_template_name, conditions, unit, blackboard, scratchpad,
    condition_args, action_data, is_running, ability_extension, context
)
    ...
    if ability_template_name == "veteran_combat_ability" then
        return _can_activate_veteran_combat_ability(context, thresholds, {
            conditions = conditions, unit = unit, blackboard = blackboard,
            scratchpad = scratchpad, condition_args = condition_args,
            action_data = action_data, is_running = is_running,
...
- [ ] **Step 5: Simplify `TEMPLATE_HEURISTICS` dispatch table**

The dispatch table entries collapse from 9-arg wrappers to direct function references:
...
Functions without threshold tables (broker_focus, broker_rage) keep `(context)` and ignore the `nil` thresholds — `_evaluate_template_heuristic` calls `fn(context, thresholds)` which passes `nil` as second arg; those functions' signatures are `(context)` so the extra arg is silently ignored by Lua.
...
local function evaluate_heuristic(template_name, context, opts)
    ...
    if template_name == "veteran_combat_ability" then
        ...
        local can_activate, rule = _can_activate_veteran_combat_ability(context, thresholds, {
            conditions = opts.conditions or {},
            unit = opts.unit,
            blackboard = nil, scratchpad = nil, condition_args = nil,
            action_data = nil, is_running = false,
            ability_extension = opts.ability_extension,
        })
...
git commit -m "feat(heuristics): per-heuristic threshold tables + (context, thresholds) signature (#6)
...
9-arg dispatch signature collapses to (context, thresholds).
...
enable_stealth_description = { en = "Invisibility abilities (Zealot Invisibility)" },
