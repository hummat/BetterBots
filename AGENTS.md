...
**Scope — critical:** BetterBots is client-side and only affects bots in locally-hosted sessions. SoloPlay missions are identified by `get_mod("SoloPlay").is_soloplay()` or `Managers.multiplayer_session:host_type() == "singleplay"`/`"singleplay_backend_session"`; `game_mode_settings.host_singleplay = true` is only a fallback and is false in some real SoloPlay mission logs. Darktide uses Fatshark dedicated servers for all public and matched play, so this mod has zero effect on bots there. In practice, the Solo Play mod is the only context where BetterBots runs. When answering "does the mod work in [Havoc / public / matched / Penances / X]" questions, the answer is **no** unless X is locally hosted — verify against the local singleplayer host type before investigating engine capabilities. Do not pattern-match from Vermintide 2's P2P/listen-server model; Darktide's architecture is different.
...
**Mock fidelity rule:** Test mocks must only expose methods and fields verified to exist on the real engine class — via decompiled source (`../Darktide-Source-Code/`) or in-game dump. This applies to all mocked surfaces: `ScriptUnit.has_extension`/`ScriptUnit.extension` doubles, manager-system doubles (`Managers.state.extension:system(...)`), and direct engine object fields (`Side`, `BotGroup`, `GroupSystem`, etc.). Darktide has extension subtype splits where the same `system_name` returns different classes for players vs minions (e.g. `unit_data_system` → `PlayerUnitDataExtension` with `read_component` for players, `MinionUnitDataExtension` with only `breed()` for enemies), and server-only initialization where fields exist on the server instance but are nil on dedicated-server clients (e.g. `GroupSystem._bot_groups`, `Side.ai_target_units` population). Mocks that expose nonexistent fields or assume server-side state create false test confidence — tests pass, production crashes or silently misbehaves. When code can receive both player and minion units, test both paths. When code can run on both server and client, verify field availability for both. See #95. Current audited surface + source-line evidence: `docs/dev/mock-api-audit.md`.
