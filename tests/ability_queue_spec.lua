...
describe("ability_queue", function()
	...
	describe("template fallback fast paths", function()
		...
		it("fails closed instead of erroring when the action_input_system extension is missing", function()
			saved_script_unit = _G.ScriptUnit
			saved_require = require
			...
			_G.ScriptUnit = {
				...
				extension = function(_, system_name)
					...
					error("ScriptUnit.extension: unit has no " .. tostring(system_name))
				end,
			}
			rawset(_G, "require", function(path)
				if path == "scripts/settings/ability/ability_templates/ability_templates" then
		...
		it("logs the first movement-ability nav block but suppresses cached repeats", function()
			...
			rawset(_G, "require", function(path)
				if path == "scripts/settings/ability/ability_templates/ability_templates" then
					return {
						zealot_dash = {
							ability_meta_data = {
								activation = { action_input = "aim_pressed" },
							},
						},
					}
				end
			...
			AbilityQueue.init({
				...
				equipped_combat_ability = function()
					return ability_extension, { name = "zealot_targeted_dash" }
				end,
				equipped_combat_ability_name = function()
					return "zealot_targeted_dash"
				end,
				is_suppressed = function()
					return false
				end,
			...
			AbilityQueue.wire({
				Heuristics = {
					resolve_decision = function()
						return true, "zealot_dash_priority_target", { num_nearby = 2 }
					end,
				},
				MetaData = { inject = function() end },
				ItemFallback = {
		...
		it("validates fallback rescue geometry before mutating aim", function()
			...
			local bot_unit_input = test_helper.make_bot_unit_input({
				...
				set_aim_position = function(_, position)
					call_order[#call_order + 1] = "set_aim_position"
					assert.same(ally_pos, position)
				end,
			})
