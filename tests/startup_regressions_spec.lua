...
describe("startup regressions", function()
	...
	it("keeps settings UI organized through behavior-focused groups", function()
		local handle = assert(io.open("scripts/mods/BetterBots/BetterBots_data.lua", "r"))
		local source = assert(handle:read("*a"))
		handle:close()
		...
		assert.is_truthy(source:find("local function make_checkbox", 1, true))
		local team_setup_index = assert(source:find('setting_id = "bot_profiles_group"', 1, true))
		local abilities_index = assert(source:find('setting_id = "abilities_group"', 1, true))
		local combat_index = assert(source:find('setting_id = "bot_feature_toggles_group"', 1, true))
		local support_index = assert(source:find('setting_id = "bot_tuning_group"', 1, true))
		local diagnostics_index = assert(source:find('setting_id = "diagnostics_group"', 1, true))
		assert.is_true(team_setup_index < abilities_index)
		assert.is_true(abilities_index < combat_index)
		assert.is_true(combat_index < support_index)
		assert.is_true(support_index < diagnostics_index)
		assert.is_nil(source:find('setting_id = "healing_deferral_group"', 1, true))
		...
		assert.is_truthy(source:find('setting_id = "bot_survivability_profile"', 1, true))
		...
		loc_handle:close()
		assert.is_truthy(localization:find('en = title("Combat Abilities")', 1, true))
		...
		assert.is_nil(localization:find("vanilla"))
		assert.is_nil(localization:find("Game default", 1, true))
		assert.is_nil(localization:find("Bot Tweaks", 1, true))
		assert.is_nil(localization:find("Bot Tuning", 1, true))
		assert.is_nil(localization:find("Healing Priority", 1, true))
	end)
	...
	describe("widget parity guard", function()
		...
		it("every widget setting_id has a matching Settings.DEFAULTS entry and vice versa", function()
			...
			for setting_id in data_source:gmatch('make_checkbox%(%s*"([^"]+)"') do
				widget_ids[setting_id] = true
			end
